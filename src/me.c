...
void ME_ModeTransition (uint8_t modeToEnter);
// ****************************************************************************
...
void ME_ModeTransition (uint8_t modeToEnter){
   ...
   //enter to mode (0 = reset, 2 = safe, 3 = drun, 4 = run0, 5 = run1)
    while( ME.GS.B.S_CURRENTMODE != modeToEnter){
	ME.MCTL.R = (modeToEnter<<28) | 0x00005AF0;
	ME.MCTL.R = (modeToEnter<<28) | 0x0000A50F;
	while (ME.GS.B.S_MTRANS!=0); // wait for transition to finish and then check
	ME.IS.B.I_TC   = 1;            //clear MTC (mode transition completed) bit
	ME.IS.B.I_CONF   = 1;
    }
   //correct mode transition finished
...
void Clock_Init(){
    ...
    // !!! Enable all peripheral clocks - added for LEOPARD !!!
    CGM.SCDC0.R = 0x80000000; //system clock divider
    //Disable both CMUs (it disables PLL frequency check)
    CGM.CMU_0_CSR.B.CME_A = 0;
    CGM.CMU_1_CSR.B.CME_A = 0;
    //Setup system PLL (PLL0) used by FlexCAN
    //set PLL to 48Mhz from 16Mhz internal clock
    CGM.FMPLL[0].CR.R = 0x03200012; // just init will be overridden next
    CGM.FMPLL[0].CR.B.IDF  = 0x2;       // Divide by 3
    CGM.FMPLL[0].CR.B.ODF  = 0x1;       // 0x0 Divide by 2; 0x1 Divide by 4; 0x2 Divide by 8; 0x3 Divide by 16;
    CGM.FMPLL[0].CR.B.NDIV = 36;        // LDF
    //selecting the internal oscillator as input to the FMPLL
    CGM.AC3SC.B.SELCTL = 0; // AUX Clock selector 3
    CGM.AC4SC.B.SELCTL = 0; // AUX Clock selector 4
...
//    ME.DRUN.R       = 0x001F0070 | 0; //SYSCLK = 0 (IRC 16M), = 2 (4MHz crystal oscilator), = 4(System PLL)
    ...
    ME.RUNPC[1].R   = 0x1E; //enable only in RUN0 and DRUN modes
//    ME.LPPC[1].R    = 0x00; //disable when in a low power mode
    ...
    ME.RUNPC[0].R   = 0x1E; //enable only in RUN0 and DRUN modes
//	ME.LPPC[0].R    = 0x00; //disable when in a low power mode
    ...
    ME.PCTL[ME_PCTL_FLEXCAN0].R = 0; //DBG_F = 0, LP_CFG = 0, RUN_CFG = 0
    ME.PCTL[ME_PCTL_PIT].R = 0; //DBG_F = 0, LP_CFG = 0, RUN_CFG = 0
    ME.PCTL[68].R = 0; // activate SIUL module
    //ME.PCTL[ME_PCTL_SAFETYPORT].B.RUN_CFG = 1;       // enable Safety port needed for correct mode transition

    Clock_Init();
    ME_ModeTransition (3);	//enter DRUN to make changes above valid
    ME_ModeTransition (4);	//enter RUN0 to make changes above valid
