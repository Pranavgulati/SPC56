...
{
	__asm__ volatile ("" \
	".equ  INTC_IACKR2, 0xFC040028  # Interrupt Acknowledge Register address					\n\t" \
	...
	"prolog:                     		# PROLOGUE 											\n\t" \
	    ...
	    __asm__ volatile ("" \
   		".equ  INTC_EOIR2,  0xFC040038  # End Of Interrupt Register address					\n\t" \
		"epilog:                    	# EPILOGUE											\n\t" \
		...
		"eaddi       r1,r1,(4*20)   	# Restore space on stack							\n\t" \
...
/**************************************************************************/
__attribute__((section(".isrTrapTable"),naked))
...
const uint32 IsrVectorTable[] __attribute__ ((section(".interrupt_table"),aligned (2048))) = {
	...
	(uint32) &dummy,	// IRQ 060, PI timer channel 1
